...
export function formatDataForUI(positions: CollectedData[]) {
    const dataForUI = positions.map((data, index) => {
        ...
        return {
            ...
            } ${data.position.token0Symbol}/${data.position.token1Symbol}`,
            token1token0Price: `${
                fromsqrtPriceX96ToHumanReadable(
                    data.pool.sqrtPriceX96,
                    data.position.token0Decimals,
            ...
            } ${data.position.token1Symbol}/${data.position.token0Symbol}`,
            ...
            mintSupplyToken0: ethers.formatUnits(
                Number(data.positionMintInfo.token0Amount),
                Number(data.position.token0Decimals)
            ),
            mintSupplyToken1: ethers.formatUnits(
                Number(Number(data.positionMintInfo.token1Amount)),
                Number(data.position.token1Decimals)
            ),
            isInRange: inRange,
        }
